...
class ResourceGathering(gym.Env, EzPickle):
    ...
    - 0: +1 if he finds and befriends a dragon, else 0
    - 1: +1 if returned home with gold, else 0
    - 2: +1 if returned home with diamond, else 0

    ## Starting State
    The agent starts at the home position with no gold, diamond or dragon friends.

    ## Episode Termination
    The episode terminates when the agent returns home (NOT TRUE: , or when 100 steps are exceeded.)
    ...
    def __init__(self, render_mode: Optional[str] = None): #"rgb_array"): 
        ...
        self.initial_pos = np.array([4, 0], dtype=np.int32)
        ...
        self.observation_space = Box(low=0, high=5, shape=(2,), dtype=np.int32)

        # action space specification: 1 dimension, 0 up, 1 down, 2 left, 3 right
        self.action_space = Discrete(4)
        # reward space:
        self.reward_space = Box(low=np.array([0]), high=np.array([1]), shape=(1,), dtype=np.int32)
        self.reward_dim = 1
    ...
    def step(self, action):
        ...
        vec_reward = np.zeros(1, dtype=np.int32)
        done = False

        cell = self.get_map_value(self.current_pos)
        if cell == "R1" and not self.has_gold:
            self.has_gold = 1
            vec_reward[0] = self.has_gold
        elif cell == "R2" and not self.has_gem:
            self.has_gem = 1
            vec_reward[2] = self.has_gem
        elif (cell == "E1" or cell == "E2") and not self.has_friend:
            self.has_friend = 1
